...
export const WinnerDisplay: React.FC = () => {
  const { winner, clearCounter } = useSimpleSSE();
  const [isVisible, setIsVisible] = useState(false);

  // Mostrar o modal quando há um winner
  useEffect(() => {
    if (winner) {
      setIsVisible(true);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="glass rounded-2xl p-8 max-w-lg mx-4 text-center border border-white border-opacity-20">
